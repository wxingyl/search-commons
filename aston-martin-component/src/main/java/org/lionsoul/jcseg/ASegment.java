...
//				mList.add(dic.get(ILexicon.CJK_WORD, _key));
                IWord scrWord = dic.get(ILexicon.CJK_WORD, _key);
                IWord nWord = new Word(_key, ILexicon.CJK_WORD);
                nWord.setSyn(scrWord.getSyn());
                nWord.setPinyin(scrWord.getPinyin());
                nWord.setPosition(scrWord.getPosition());
                nWord.setPartSpeech(scrWord.getPartSpeech());
                mList.add(nWord);
            }
        }
...
/*		for ( int j = 0; j < mList.size(); j++ ) {
    ...
    public boolean findCHName(IWord w, IChunk chunk) {
        String s1 = w.getValue().charAt(0) + "";
        String s2 = w.getValue().charAt(1) + "";
            ...
            switch (sec.getLength()) {
                case 1:
                    if (dic.match(ILexicon.CN_DNAME_2, sec.getValue()))
                        return true;
                case 2:
                    String d1 = sec.getValue().charAt(0) + "";
        ...
        if (!_check) {
            ...
            if (chkunits && (ENSCFilter.isDigit(__str)
                ...
                if (dic.match(ILexicon.CJK_UNITS, ((char) ch) + "")) {
                    w = new Word(__str + ((char) ch), IWord.T_MIXED_WORD);
                    w.setPartSpeech(IWord.NUMERIC_POSPEECH);
                } else pushBack(ch);
            }
