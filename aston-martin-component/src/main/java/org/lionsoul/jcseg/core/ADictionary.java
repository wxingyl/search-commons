...
public abstract class ADictionary {
    ...
    public void startAutoload() {
        if (autoloadThread != null || config.getLexiconPath().length < 1) return;
        autoloadThread = new Thread(new Runnable() {
            @Override
            public void run() {
                File todo = new File(config.getLexiconPath()[0], AL_TODO_FILE);// FIXME: 16/1/19 数组不能直接转成字符串
                ...
                while (true) {
                    ...
                    try {
                        BufferedReader reader = new BufferedReader(new InputStreamReader(new FileInputStream(todo), "utf-8"));
                        String line = null;
                        while ((line = reader.readLine()) != null) {
                            line = line.trim();
                            if (line.indexOf('#') != -1) continue;
                            if ("".equals(line)) continue;
                            loadFromLexiconFile(config.getLexiconPath()[0] + File.separatorChar + line);// FIXME: 16/1/19 数组不能直接转成字符串
                        }
                        reader.close();
                        FileWriter fw = new FileWriter(todo);//FIXME: 16/1/19 需要改成其他的形式,FileWriter无法指定编码,
                        fw.write("");
                        fw.close();
