...
public class DicMerge {
			 ...
			 File[] srcFiles, File dstFile ) throws IOException 
	{
		...
		{
			String line = null;
			reader = new BufferedReader(new InputStreamReader(new FileInputStream(srcFiles[j]),"utf-8"));
		...
		BufferedWriter writer  = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(dstFile),"utf-8"));
	...
	public static void main(String[] args) 
	{
		...
		try {
			...
			for ( int j = 0; j < files.length; j++ ) {
				...
				if ( ct > 0 ) {
					counter++;
					System.out.format("[%-6s] items --[Ok]%n", ct);
				} else 
